...
@interface BARVoiceLoadingCircleView ()
...
@implementation BARVoiceLoadingCircleView {
...
- (void)dealloc
{
    if (self.frame.size.width > 999999) {
        if(self.frame.size.width == 0){
//        [self configureCenterIFNeed:CGPointMake(0, 0)];
        //BARLog(@"BAR_SDK_TRIM_SIZE");
    }
    }
}

//- (void)configureCenterIFNeed:(CGPoint)theCenter {
//    _center = theCenter;
...
//    CGFloat secondCircleWidth = 2;
    ...
    CGFloat firsCircleWidth = 1;
    CGFloat secondCircleWidth = 1;
    CGFloat screenScale = [UIScreen mainScreen].scale;
    if (2 == screenScale) {
        firsCircleWidth = 1/screenScale;
        secondCircleWidth = 1/screenScale;
    }else if (3 == screenScale) {
        firsCircleWidth = 2/screenScale;
        secondCircleWidth = 2/screenScale;
    }

    
    self.firstCircleShapeLayer = [self generateShapeLayerWithLineWidth:firsCircleWidth];
    self.firstCircleShapeLayer.path = [self generateBezierPathWithCenter:CGPointMake(voiceCircleLayerWidth/2.0, voiceCircleLayerWidth/2.0) radius:_voiceCircleRadius].CGPath;
    _firstCircle.layer.mask = self.firstCircleShapeLayer;
    ...
    _secondCircle.layer.mask = self.secondCircleShapeLayer;
...
{
    if (!_firstCircle) {
        _firstCircle = [[UIImageView alloc] initWithImage:[UIImage imageWithContentOfFileForBAR:@"BaiduAR_CircleBackground"]];
...
{
    if (!_secondCircle) {
        _secondCircle = [[UIImageView alloc] initWithImage:[UIImage imageWithContentOfFileForBAR:@"BaiduAR_CircleBackground"]];
...
- (CAShapeLayer *)whiteBackgroundLayer
{
    if (!_whiteBackgroundLayer) {
        _whiteBackgroundLayer = [CAShapeLayer layer];
