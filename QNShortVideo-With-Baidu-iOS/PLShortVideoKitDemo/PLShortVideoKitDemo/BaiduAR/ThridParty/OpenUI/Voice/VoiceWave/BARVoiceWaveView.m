...
@property (nonatomic, strong) BARVolumeQueue *volumeQueue;
...
@implementation BARVoiceWaveView {
...
- (void)showInParentView:(UIView *)parentView frame:(CGRect)frame
{
...
//    self.frame = CGRectMake(0, parentView.bounds.size.height * 0.25, parentView.bounds.size.width, parentView.bounds.size.height * 0.5);
    ...
    CGFloat firstLineWidth = 1;
    CGFloat secondLineWidth = 1;
    CGFloat screenScale = [UIScreen mainScreen].scale;
    if (2 == screenScale) {
        firstLineWidth = 1/screenScale;
        secondLineWidth = 1/screenScale;
    }else if (3 == screenScale) {
        firstLineWidth = 2/screenScale;
        secondLineWidth = 2/screenScale;
    }
    self.firstShapeLayer = [self generateShapeLayerWithLineWidth:firstLineWidth];
    _firstLine.layer.mask = self.firstShapeLayer;
    ...
    _secondLine.layer.mask = self.secondShapeLayer;
...
{
    if (!_firstLine) {
        _firstLine = [[UIImageView alloc] initWithImage:[UIImage imageWithContentOfFileForBAR:@"BaiduAR_FirstLine"]];
...
{
    if (!_secondLine) {
        _secondLine = [[UIImageView alloc] initWithImage:[UIImage imageWithContentOfFileForBAR:@"BaiduAR_SecondLine"]];
...
{
    if (!_fillLayerImage) {
        _fillLayerImage = [[UIImageView alloc] initWithImage:[UIImage imageWithContentOfFileForBAR:@"BaiduAR_FirstLine"]];
...
{
    if (!_fillShapeLyer) {
        _fillShapeLyer = [CAShapeLayer layer];
...
- (UIImageView *)selfViewMaskImage
{
    if (!_selfViewMaskImage) {
        ...
        _selfViewMaskImage.image = [UIImage imageWithContentOfFileForBAR:@"BaiduAR_MaskImage"];
...
- (BARVolumeQueue *)volumeQueue
{
    if (!_volumeQueue) {
        _volumeQueue = [[BARVolumeQueue alloc] init];
    }
